...
@section('content')
    <div class="card mb-4">
        <div class="card-header d-flex justify-content-between align-items-center">
            <div>
                ...
                <small class="text-muted">
                    {{ $invoice->invoice_date->format('M d, Y') }} • {{ ucfirst($invoice->type) }}
                </small>
            </div>
            <div class="d-flex gap-2 align-items-center">
                {!! $invoice->status_badge !!}
                <a href="{{ route('admin.invoices.download', $invoice) }}" class="btn btn-outline-secondary btn-sm">
        ...
        <div class="card-body">
            {{-- Billing block similar to client invoice show --}}
            <div class="row mb-4">
                <div class="col-md-6">
                    <h6 class="text-muted">Billed To</h6>
                    <p class="mb-0 fw-semibold">
                        {{ $invoice->billing_address['company'] ?? $invoice->user->company_name ?? $invoice->user->name }}
                    </p>
